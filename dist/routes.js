...
export class AuthConflictError extends Error {
    constructor(msg) { super(msg); this.name = 'AuthConflictError'; }
}
...
export function createAuthRoutes(deps) {
    ...
    auth.post('/api/auth/login', async (c) => {
        ...
        if (!email || !password) {
            if (isFormSubmit)
                return c.redirect(`${dashboardUrl}/login?error=missing_fields`);
            return c.json({ error: 'missing_fields' }, 400);
        }
        const user = await deps.repo.findUserByEmail(email);
        if (!user) {
            if (isFormSubmit)
                return c.redirect(`${dashboardUrl}/login?error=invalid_credentials`);
        ...
        if (isFormSubmit)
            return c.redirect(`${dashboardUrl}/workspaces`);
...
function createRequireAuthFromToken(tokenService, cookieName) {
